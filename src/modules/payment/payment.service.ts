...
// =============================================
const createPayment = async (tenantId: string, payload: any) => {
  if (!payload) {
    ...
    if (existingPayment.status === "PENDING") {
      ...
      return {
        ...
        stripe: {
          ...
          sessionId:
            paymentDetails?.stripeSessionId || existingPayment.transactionId,
  ...
  return {
    message:
      "Payment created successfully. Please complete the payment process.",
    payment: {
      ...payment,
...
// 3. Payment Cancel
// =============================================
const paymentCancel = async (sessionId: string) => {
  if (!sessionId) {
    throw new Error("Session ID is required");
  }
...
// =============================================
const getMyPayments = async (userId: string, userRole: string, query: any) => {
  if (!userId) {
    throw new Error("User ID is required");
  }
...
// =============================================
const getPaymentStats = async (userId: string, userRole: string) => {
  if (!userId) {
    throw new Error("User ID is required");
  }
  ...
  const [total, pending, paid, failed, refunded, totalAmount] =
    await Promise.all([
